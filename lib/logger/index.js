...
/**
 * 保证有一个 console transport，如果有多个，那么  第一个有有效
 */
Logger.prototype._checkConsoleTransport = function() {
    ...
    if (consoleTypeFirstIndex === -1) {
        ...
        this.config.remove(function(n, index) {
            return n.type && n.type === "console" && index !== consoleTypeIndex;
        });
    }
}
...
Logger.prototype.load = function() {
    ...
    //
    // 1. 如果 transport 设置了 level，那么 logger.level <= transport.level  都显示
    //    [src:]  transport.level && self.levels[transport.level] >= self.levels[level]
    //
    // 2. 如果没设置 transport 的 level，那么 logger.level <= logger.options.level || 'info'
