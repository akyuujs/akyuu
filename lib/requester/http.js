...
var USER_AGENT = ("AkyuuHttpRequester/{version} ({platform} {release}_{arch}; N; zh_CN) " +
    "Spidex/{spidexVersion} Akyuu {version}").assign({
        version: pkg.version,
...
HTTPRequester.prototype.post = function(router, params, callback) {
    ...
    spidex.post(uri, {
        ...
        if(result instanceof Error) return callback(new Errors.ThirdParty(result));
        callback(undefined, result, header, status);
    }).on('error', function(err) {
        return callback(new Errors.ThirdParty(err));
    });
};
